...
def showText(img, numberPlate, datePlate, code, city, oddEven, month, year):
    ...
    cv2.putText(img, numberPlate, (20, img.shape[0] - int(2.1 * distance)), cv2.FONT_HERSHEY_PLAIN, scale, color,
                thickness)
    cv2.putText(img, "City: " + str(city), (20, img.shape[0] - int(1.5 * distance)), cv2.FONT_HERSHEY_PLAIN, 1.5,
                colorG, thickness)
    cv2.putText(img, "Type: " + oddEven, (20, img.shape[0] - 1 * distance), cv2.FONT_HERSHEY_PLAIN, 1.5, colorG,
                thickness)
    cv2.putText(img, textExpired, (20, img.shape[0] - int(0.5 * distance)), cv2.FONT_HERSHEY_PLAIN, 1.5, colorG,
                thickness)
    cv2.putText(img, cautionStatus, (20 + 135, img.shape[0] - int(1 * distance)), cv2.FONT_HERSHEY_PLAIN, 1, colorR,
                thickness)
    cv2.putText(img, cautionExpired, (20 + 225, img.shape[0] - int(0.5 * distance)), cv2.FONT_HERSHEY_PLAIN, 1, colorR,
                2)
